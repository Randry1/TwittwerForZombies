class ZombiesController < ApplicationController
  ...
  #filter zombie
  def filter
    @zombies = Zombie.younger
  end

  def decomp
    @zombie = Zombie.find(params[:id])
    @zombie_to_json = @zombie.to_json(include: :brain, only: [:name, :age, :decomp, :email, :bio, :id])
    if @zombie.decomp = 'Dead(again)'
      render json: @zombie_to_json, status: :unprocessable_entity
    else
      render json: @zombie_to_json, status: :ok
    end
  end
